...
export class ModalAddServicePointComponent implements OnInit {
  ...
  set setItems(value: any) {
    ...
    this.roomQueueRunning = value.room_queue_running === 'Y' ? true : false;
  }
  ...
  async save() {
    if (this.servicePointName && this.localCode && this.prefix && this.departmentId) {
      try {
        ...
        var roomQueueRunning = this.roomQueueRunning ? 'Y' : 'N';
        const data: any = {
          ...
          roomQueueRunning: roomQueueRunning
        };
