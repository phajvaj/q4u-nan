...
@Component({
  ...
  templateUrl: './display-queue-room.component.html',
  styles: [
    `
    .main-panel {
...
export class DisplayQueueRoomComponent implements OnInit, OnDestroy {
  ...
  connectWebSocket() {
    ...
    this.client.on('connect', () => {
      console.log('Que Room Connected!');
      that.zone.run(() => {
  ...
  async getCurrentQueue() {
    ...
    if (this.room5.room_id) {
      try {
        const rs: any = await this.queueService.getQueRoom(this.room5.room_id, this.token);
        if (rs.statusCode === 200) {
          this.room5Items = rs.results;
        } else {
          console.log(rs.message);
          this.alertService.error('เกิดข้อผิดพลาด');
        }
    ...
    if (this.room6.room_id) {
      try {
        const rs: any = await this.queueService.getQueRoom(this.room6.room_id, this.token);
        if (rs.statusCode === 200) {
          this.room6Items = rs.results;
        } else {
          console.log(rs.message);
          this.alertService.error('เกิดข้อผิดพลาด');
        }
