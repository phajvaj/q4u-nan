...
@Component({
  selector: 'app-display-queue-rooms',
  templateUrl: './display-queue-rooms.component.html',
  styles: [
    `
    .thai-sarabun {
...
export class DisplayQueueRoomsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.route.queryParams
      .subscribe(params => {
        ...
        if (params.roomId1) {
          this.room1 = params.roomId1;
        }
        if (params.roomId2) {
          this.room2 = params.roomId2;
        }
        if (params.roomId3) {
          this.room3 = params.roomId3;
        }
        if (params.roomId4) {
          this.room4 = params.roomId4;
        }
      });
  ...
  async ngOnInit() {
    try {
      ...
      if (token) {
        ...
        let _col = 0;
        if (this.room1) {
          this.roomForget = true;
          this.room1 = await this.getRoomInfo(this.room1);
          _col++;
        }

        if (this.room2) {
          this.room2 = await this.getRoomInfo(this.room2);
          _col++;
        }

        if (this.room3) {
          this.room3 = await this.getRoomInfo(this.room3);
          _col++;
        }

        if (this.room4) {
          this.room4 = await this.getRoomInfo(this.room4);
          _col++;
        }

        this._cols = Math.round(12 / _col);
      ...
      this.alertService.error('ngOnInit เกิดข้อผิดพลาด');
      console.log(error);
    }
  }
  ...
  async onSelectedPoint(event: any) {
    ...
    if (!this.roomForget)
      await this.getRoom(this.servicePointId);
  ...
  async getRoomInfo(roomId) {
    try {
      const rs: any = await this.queueService.getRoomInfo(roomId, this.token);
  ...
  async getRoom(servicePointId) {
    try {
      const rs: any = await this.queueService.getRoom(servicePointId, this.token);
      let len = 0;
      if (rs.statusCode === 200) {
        for (let i = 0; i < 4; i++) {
          this['room' + (i + 1)] = rs.results[i] || null;
          if (rs.results[i]) len++;
        }
        this._cols = Math.round(12 / len);
      }
    } catch (error) {
      console.log(error);
  ...
  async getCurrentQueue() {
    try {
      const rs: any = await this.queueService.getWorking2(this.servicePointId, this.token);
      if (rs.statusCode === 200) {
      ...
      if (this.room1) {
        try {
          const rs: any = await this.queueService.getQueRoom(this.room1.room_id, this.token);
          if (rs.statusCode === 200) {
            this.room1Items = rs.results;
          } else {
            console.log(rs.message);
            this.alertService.error('เกิดข้อผิดพลาด');
          }
      ...
      if (this.room2) {
        try {
          const rs: any = await this.queueService.getQueRoom(this.room2.room_id, this.token);
          if (rs.statusCode === 200) {
            this.room2Items = rs.results;
          } else {
            console.log(rs.message);
            this.alertService.error('เกิดข้อผิดพลาด');
          }
      ...
      if (this.room3) {
        try {
          const rs: any = await this.queueService.getQueRoom(this.room3.room_id, this.token);
          if (rs.statusCode === 200) {
            this.room3Items = rs.results;
          } else {
            console.log(rs.message);
            this.alertService.error('เกิดข้อผิดพลาด');
          }
      ...
      if (this.room4) {
        try {
          const rs: any = await this.queueService.getQueRoom(this.room4.room_id, this.token);
          if (rs.statusCode === 200) {
            this.room4Items = rs.results;
          } else {
            console.log(rs.message);
            this.alertService.error('เกิดข้อผิดพลาด');
          }
